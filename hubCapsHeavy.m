function [ HeavyHubs ] = hubCapsHeavy( Metrics  )
%HUBCAPSHEAVY Calculates group or individual hub measures
%
%   [HeavyHubs] = hubCapsHeavy(Metrics);
% 
%   Weighted - also binary version
%
%   Total of 8 nodal measures
%   Runs best with heavyMetrics = myHeavyMeasures(CIJ);
%
...
st = Metrics.strength; %strength
cl = Metrics.closeness; %closeness centrality
bc = Metrics.node_betweenness; %node betweenness centrality
...
if max(rem(deg,1)) == 0
    ...
    error('input is binary - please use binary version')
end
...
degHub = zeros(m,1); stHub = zeros(m,1); clHub = zeros(m,1); 
bcHub = zeros(m,1); zHub = zeros(m,1); pHub = zeros(m,1); 
vHub = zeros(m,1); prHub = zeros(m,1);
...
HeavyHubs.individual = hub_score;
HeavyHubs.metrics = [degHub stHub clHub bcHub zHub pHub vHub prHub];
